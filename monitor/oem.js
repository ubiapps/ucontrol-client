(function() {
  ...
  emon.prototype = Object.create(eventEmitter.prototype);
  emon.prototype.constructor = emon;

  emon.prototype.start = function() {
    var self = this;
    ...
    this._serialPort.open(function(err) {
      if (typeof err !== "undefined" && err !== null) {
        console.log("emon - failed to open port " + self._portName + " - " + JSON.stringify(err));
      } else {
        utils.logger.info("emon - opened port");

        self._serialPort.on("error", function(e) {
          utils.logger.info("emon - port error: " + JSON.stringify(e));
        });

        self._serialPort.on("data", function (data) {
          if (typeof data !== "undefined" && data !== null) {
            utils.logger.info("emon: " + data);
